...
knitr::include_graphics("dnaseseq.png")
```
...
---
## Cytosine methylation --- biology

* Cytosine methylation regulates X chromosome inactivation in most female mammals

--

* It also regulates imprinted genes which express only from one of their two copies

--

* With exceptions during development, most CpG islands at the promoter of genes do not show methylation across different tissues

-- 

* CpG islands often coincide with active promoters of germ cells and DNA replication origins


---
## Cytosine methylation --- function
...
## Cytosine methylation --- evolution

* Methylated cytosine could be deaminated to Uracil without being recognized by DNA repair machinery

--

* Global demethylation of primordial germ cells sustains the lifetime of CpG islands through generations

--

* A hypothesis explaining the evolution of CpG islands suggests that their vicinity to origins of replication hints the involvement of DNA polymerase in expanding these regions in the germ cells
...
```{bash eval=FALSE}
/scratch/t/teachmmg3003/mmg3003ta002
```

* You can copy the fastq files from that directory to your scratch folder:

```{bash eval=FALSE}
cp -rf /scratch/t/teachmmg3003/mmg3003ta002/newFastqFilesChr22 $SCRATCH
```
...
cat $SCRATCH/Ref/hg38.chromsizes |  sed 's/^chr//' > $SCRATCH/Ref/GRCh38.chromsizes
```
...
do
  ...
  ~/software/bin/bedGraphToBigWig $MAINDIR/$SAMPLE/$BDG $SCRATCH/Ref/GRCh38.chromsizes $MAINDIR/$SAMPLE/$BW
done
```
